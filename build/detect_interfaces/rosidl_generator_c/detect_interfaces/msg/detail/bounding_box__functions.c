  ...
  detect_interfaces__msg__BoundingBox__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    detect_interfaces__msg__BoundingBox * data =
      (detect_interfaces__msg__BoundingBox *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!detect_interfaces__msg__BoundingBox__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          detect_interfaces__msg__BoundingBox__fini(&output->data[i]);
        }
        return false;
      }
    }
